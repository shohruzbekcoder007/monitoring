class MainClass{
    ...
    async elementOne(req){
        try{
            let element = await this.Schema.findOne({_id: req.body.customer_id})
            return element;
        } catch(ex){
            return ex;
        }
    ...
    async elementDelete(req){
        try{
            let removElement = await this.Schema.remove({_id: req.body.customer_id});
            return removElement;
        } catch(ex){
            return ex;
        }
    ...
    async elementUpdate(req){
        try{
            let updateElement = await this.Schema.update({_id: req.body.customer_id},{$set: req.body.updateData});
